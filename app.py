...
def render_admin_dashboard():
    ...
    with st.expander("Add a New Question", expanded=True):
        with st.form("add_question_form", clear_on_submit=True):
            ...
            if submitted:
                options = [opt.strip() for opt in [opt1, opt2, opt3, opt4] if opt.strip()]
                if new_q.strip() and len(options) >= 2:
                    ...
                    st.error("Please provide a question and at least 2 valid options.")

    st.subheader(f"Current Questions Bank ({len(st.session_state.questions)})")
    
    for idx, item in enumerate(st.session_state.questions):
        cols = st.columns([4, 1])
        with cols[0]:
            st.markdown(f"**Q{idx + 1}: {item['q']}**")
            st.caption(f"Options: {', '.join(item['options'])} | Correct: `{item['ans']}`")
        with cols[1]:
...
def render_student_quiz():
    ...
    if not st.session_state.quiz_finished:
        ...
        choice = st.radio("Select an answer:", current_q["options"], key=f"q_{current_idx}")
...
def main():
    st.set_page_config(page_title="Dynamic Quiz Platform", layout="centered")
    init_session_state()

    st.sidebar.title("Navigation")
    role = st.sidebar.radio("Select Role:", ["Student Portal", "Admin Dashboard"])
